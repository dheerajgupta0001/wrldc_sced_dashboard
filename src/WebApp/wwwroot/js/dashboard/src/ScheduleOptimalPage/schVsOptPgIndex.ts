...
window.onload = async () => {
    ...
    for (let i = 0; i < fetchedGeneratorsData.length; i++) {
        ...
        option.text = fetchedGeneratorsData[i].genName;
    ...
    if (startDateValue === "" || endDateValue === "") {
        ...
        for (let genInd = 0; genInd < selectedGeneratorsList.length; genInd++) {
            ...
            let plotData: PlotData = {
                title: `${selectedGeneratorsList[genInd].genName} Schedule Vs Optimal `,
                traces: [],
            };
            ...
            setPlotTraces(
                `${selectedGeneratorsList[genInd].genName}_schVsOpt`,
                plotData
            );
        }
    }
